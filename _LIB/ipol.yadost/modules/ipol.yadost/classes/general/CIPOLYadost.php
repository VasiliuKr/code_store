...
class CIPOLYadost{
	...
	static public function getDeliveryProfiles($arOrder, $arConfig)
	{
		...
		if ($res["status"] == "ok")
		{
			...
			$arProfilesConversion = self::getProfilesConversion();// дл€ конвертации названий тарифов из яƒ в битриксовые профили
			...
			// если заказ уже создан и редактируетс€, то надо подсунуть рассчитанную уже стоимость доставки
			$orderID = false;
	...
	static public function calculateOrder($params)
	{
		...
		if ($locationValue["VALUE"] && $orderData["PERSON_TYPE_ID"])
		{
			...
			// подставл€ем габариты с формы отправки за€вки
			...
			if ($calculateResult && is_array($calculateResult))
			{
				...
				foreach ($calculateResult as $res)
				{
					...
					{
						// собираем дл€ товаров заказа данные дл€ открыти€ их по ссылке
						...
						$arResult = array(
							...
							"is_ds_import_available" => $res["delivery"]["is_ds_import_available"],// само склад —ƒ
							"is_ds_withdraw_available" => $res["delivery"]["is_ds_withdraw_available"],// забор склад —ƒ
	...
	{
		// если выключено —читать стоимость доставки сразу
		if (!self::$compabilityPerform)
	...
	static public function onEpilog(){//отображение формы отправки за€вки
		CIPOLYadostHelper::checkLocationChange();
	
		// провер€ем надо ли отображать сообщени€ об изменени€х заказа и отображаем их
	...
	// провер€ет не в карточке ли заказа мы находимс€
	...
	// провер€ет не в редактировании заказа мы находимс€
	...
	static public function onBufferContent(&$content) {
		...
		if (CIPOLYadostHelper::isActive() && self::$personType && self::$selectedDelivery)
		{
			...
			// “аким вот странным способом мы передаем наши данные из PHP в js в момент Ajax-запроса.
	...
	static public function orderCreate($orderID, $orderFields)
	{
		if ($_REQUEST["yd_is_select"] == "ipolYadost")
		{
			...
			// запоминаем данные заказа, если доставка - €ндекс
			$Data = array(
				"ORDER_ID" => $orderID,
