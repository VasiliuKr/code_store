...
#        Developer: Kadrichev Dmitry
#        Site: https://ipolh.com
#################################################
?>
...
Class ipol_yadost extends CModule
{
	...
	function InstallEvents() {
		RegisterModuleDependences("sale", "OnSaleComponentOrderOneStepPersonType", $this->MODULE_ID, "CIPOLYadost", "setLocationFromCookie");// выставляем город из геовиджета
		RegisterModuleDependences("sale", "OnSaleComponentOrderOneStepOrderProps", $this->MODULE_ID, "CIPOLYadost", "loadComponent");// получаем выбранную доставку в оф заказа
		RegisterModuleDependences("main", "OnEndBufferContent", $this->MODULE_ID, "CIPOLYadost", "onBufferContent");// передаем данные из модуля в шаблон order
		RegisterModuleDependences("sale", "OnSaleComponentOrderOneStepComplete", $this->MODULE_ID, "CIPOLYadost", "orderCreate"); // записываем данные о заказе при оформлении
		RegisterModuleDependences("sale", "OnSaleComponentOrderOneStepDelivery", $this->MODULE_ID, "CIPOLYadost", "pickupLoader");
		
		RegisterModuleDependences("main", "OnEpilog", $this->MODULE_ID, "CIPOLYadost", "onEpilog"); // кнопка в заказе + форма отправки
		
		RegisterModuleDependences("sale", "OnBeforeOrderUpdate", $this->MODULE_ID, "CIPOLYadostHelper", "OnBeforeOrderUpdateHandler"); //ловим изменение заказа, чтобы вывести сообщение с предупреждением
		RegisterModuleDependences("sale", "OnOrderUpdate", $this->MODULE_ID, "CIPOLYadostHelper", "OnOrderUpdateHandler"); //ловим изменение заказа, чтобы вывести сообщение с предупреждением
		RegisterModuleDependences("sale", "OnOrderAdd", $this->MODULE_ID, "CIPOLYadostHelper", "OnOrderAddHandler"); //ловим создание заказа, чтобы вывести сообщение с предупреждением
		
		RegisterModuleDependences("sale", "OnBeforeBasketUpdate", $this->MODULE_ID, "CIPOLYadostHelper", "OnBeforeBasketUpdateHandler"); //ловим изменение корзины заказа, чтобы вывести сообщение с предупреждением
		RegisterModuleDependences("sale", "OnBasketUpdate", $this->MODULE_ID, "CIPOLYadostHelper", "OnBasketUpdateHandler"); //ловим изменение корзины заказа, чтобы вывести сообщение с предупреждением
		...
		// обновление модуля для отправки статистики
		RegisterModuleDependences("main", "OnModuleUpdate", $this->MODULE_ID, "CIPOLYadostHelper", "OnModuleUpdateHandler");
		
		// CAgent::AddAgent("CIPOLYadostDriver::agentOrderStates();", $this->MODULE_ID, "N", 1800);//обновление статусов заказов, агент ставится в step1.php
		// RegisterModuleDependences("sale", "OnSaleBeforeDeliveryOrder", $this->MODULE_ID, "CIPOLYadost", "onFlagChange");
		return true;
	}
	...
	function UnInstallEvents() {
		...
		UnRegisterModuleDependences("sale", "OnSaleComponentOrderOneStepComplete", $this->MODULE_ID, "CIPOLYadost", "orderCreate");
    ...
    function DoUninstall()
    {
		...
		// отправляем статистику на удалении модуля
//		CModule::IncludeModule($this->MODULE_ID);
//		CIPOLYadostDriver::sendStatistic(array("type" => "remove"));
